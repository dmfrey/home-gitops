...
**eGPU dock (k8s-2, AOOSTAR AG02/ASM2464 bridge) has an unresolved PCIe hotplug link problem**, independent of GPU vendor/drivers (reproduced with both an RTX 5070 and an RTX 3090) — confirmed via `talosctl dmesg --follow` on cable reseat, not authorization/udev (the `siderolabs/thunderbolt` extension already ships a working `99-thunderbolt.rules` auto-authorize rule). Failure signature varies run-to-run even with identical kernel args: sometimes full PCIe enumeration (GPU device + BARs assigned) followed by `pciehp: Timeout on hotplug command 0x1038` ~9-20s later; sometimes it never reaches PCIe hotplug at all and dies earlier at Thunderbolt retimer/tunnel negotiation. This pattern (multiple distinct failures from identical inputs) points to hardware-level flakiness on the dock's bridge chip, not something a single kernel arg fixes. `thunderbolt.host_reset=0` is the only kernel arg that's shown any measurable improvement (longer connection duration) and is the current baseline on k8s-2. `pci=realloc,assign-busses` showed no consistent benefit; `pcie_aspm=off` + `pcie_port_pm=off` measurably regressed it (fails even earlier). Dock firmware update and the GEEKOM hidden BIOS advanced menu both require a Windows-only tool — not available in this environment. See `talos/schematic.yaml.j2` inline comments for the full history before re-testing kernel args here.
