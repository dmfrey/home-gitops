...
The root justfile is `.justfile` (hidden), which declares three modules: `bootstrap`, `kube` (`kubernetes/homelab-k8s-001`), and `talos`. The internal `just template <file>` recipe renders Jinja2 templates via `minijinja-cli | op inject` (secrets injected from 1Password at render time).
...
```sh
bash scripts/kubeconform.sh kubernetes/homelab-k8s-001   # Validate all manifests with kubeconform
flux-local build ks --namespace <ns> --path kubernetes/homelab-k8s-001/flux/cluster <ks>  # Build a local Kustomization
```
...
Located in `kubernetes/homelab-k8s-001/components/`:
