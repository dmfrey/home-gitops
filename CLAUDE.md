...
- **Rook-Ceph**: Distributed block storage on NVMe SSDs across all 3 nodes. Most stateful apps use a plain `ceph-block`-backed PVC, named after the release.
- **OpenEBS**: Local storage on M.2 SATA SSDs (in Wi-Fi slot).
- **Garage**: S3-compatible object storage running in-cluster (`garage-system` namespace). Genuine 3-node replicated cluster (`replication_factor=3`, one zone per node, migrated from single-node 2026-07-25). Metadata lives on a per-node `openebs-hostpath` (local M.2 SATA SSD) PVC via `statefulset.volumeClaimTemplates` — **moved off Ceph RBD 2026-09-17** after two LMDB corruption incidents (2026-07-23, 2026-09-17) both traced to RBD network hiccups; Garage already replicates RF=3 at the application layer, so metadata never needed Ceph's network-storage resilience on top, and local per-node storage removes that whole failure class. `dependsOn: rook-ceph-cluster` was also dropped from `ks.yaml` as part of this — nothing in this app needs Ceph anymore. Object data lives on 3 **independent** NFS exports (`nas.internal:/garage`, `/garage-1`, `/garage-2`) — deliberately not one shared export split by subpath, since RF=3 replication would otherwise write 3 copies into the same underlying disk pool. Used for CNPG WAL archiving/base backups and DragonflyDB snapshots, and as **live primary storage** for Loki and Tempo (not just backup — an outage stops log/trace ingestion immediately, not just backups). When growing Garage's node count, its inherited `cluster_layout` file (in `metadata_dir`) still reflects the old `replication_factor` and Garage refuses to start until that file is moved aside — a real safety check, not a bug. **The StatefulSet requires `pod.affinity.podAntiAffinity` (`requiredDuringSchedulingIgnoredDuringExecution` on `kubernetes.io/hostname`, added 2026-07-27)** — without it, Kubernetes can freely co-locate 2+ replicas on one node, which silently defeats RF=3's fault tolerance; a single node hiccup then drops the cluster below quorum. This is exactly how a `barman-cloud` backup's catalog-list step failed with "Could not reach quorum of 2" — `garage-0` and `garage-2` had both landed on `k8s-0`. Note `requiredDuringSchedulingIgnoredDuringExecution` only affects scheduling, not already-running pods — but a Helm-triggered rolling restart (any pod template change) is enough to re-evaluate placement and fix it, no manual pod deletion needed.
    - **Before a full-cluster planned shutdown, scale `garage` (StatefulSet) to 0 replicas first** — releases its NFS mounts on `nas.internal` cleanly while the NAS is still fully responsive, before any node shutdown begins. Added to the outage runbook 2026-08-04 after node shutdowns hung with NFS mounts stuck in uninterruptible D-state once the NAS also went down concurrently; see the Talos Configuration section for the fuller finding.
    - **Single-node metadata crash recovery, in-place (logical corruption, disk itself fine)** — e.g. a Rust panic in `garage_table::gc` after an abrupt power-cycle, or LMDB `MDB_CORRUPTED: Located page was wrong type` (confirmed 2026-09-17 as the actual root cause of a `garage-0` OOMKill: a healthy peer stuck retrying large sync batches, 256 items at a time with no effective backoff, against a peer whose LMDB was corrupted, which is what actually exhausted the 1Gi memory limit, not the corruption itself — memory limit since raised to 2Gi as a defensive margin regardless): with RF=3 and the other 2 nodes healthy, safe to recover by resetting _only that node's_ local metadata, not restoring from backup. Scale the StatefulSet down by one replica (`kubectl scale statefulset garage --replicas=2` removes the highest ordinal cleanly, PVCs persist), mount its `meta-*` PVC from a throwaway debug pod, move aside (don't delete) `db.lmdb` only — **preserve `node_key`/`node_key.pub`**, since wiping those would make it rejoin as an unrecognized new node instead of the same cluster member. Scale back to 3; Garage treats the empty metadata store as a normal resync-from-peers case. **If the corrupted node isn't the highest ordinal** (e.g. `garage-1`, not `garage-2`) — StatefulSet scale-down only ever removes from the top. Scale down far enough to release its PVC (e.g. `--replicas=1` to free `garage-1`, which also stops the already-healthy `garage-2` alongside it — harmless, its own metadata is untouched) — same `db.lmdb` move-aside, then scale back to 3. Pauses all Garage writes cluster-wide (Loki/Tempo ingestion, CNPG WAL archiving) for the few minutes it takes — they queue/retry once it's back, nothing is lost.
    - **Single-node metadata replacement (volume itself is gone — disk failure, or a deliberate storage-class/PVC swap)**: don't try to preserve `node_key` across a genuinely new volume — Garage has an official node-replacement workflow for exactly this, used live 2026-09-17 for the Ceph→openebs-hostpath migration (all 3 nodes, one at a time). Scale down enough to release that ordinal's PVC (same highest-ordinal-only constraint as above), delete the old PVC, scale back up — the pod claims a **fresh, empty** volume and Garage generates a **brand-new node identity** on it (shows as `NO ROLE ASSIGNED` in `garage status`; the old identity shows under `FAILED NODES`). Then: `garage layout assign <new_id> --replace <old_id> -c <capacity> -z <zone> -t <tag>` (match the old node's exact capacity/zone/tag from `garage layout show` first), review with `garage layout show`, then `garage layout apply --version <N>`. The old identity then shows `draining metadata..` forever (it's gone, can't actually drain) — force past it with `garage layout skip-dead-nodes --version <N>`, then again with `--allow-missing-data` (safe here: the other 2 nodes still hold full RF=3 copies, this flag just accepts that the dead node's contribution to quorum bookkeeping specifically is unavailable, not that data is lost). **Watch for a scheduling race** when bringing an un-migrated (Ceph-backed, no node pinning yet) ordinal back up alongside already-migrated (local-storage, hard node-pinned) ones: whichever ordinal starts first has no placement constraint yet and can land on a node an already-migrated ordinal is pinned to, deadlocking the pod anti-affinity rule against the still-Pending one. Fix: delete the wrongly-placed pod + its just-created PVC, let the blocked one claim its correct node first, then retry the other — with the other 2 already placed, anti-affinity forces it onto the one remaining valid node.
...
- **`rook-ceph-crashcollector` cephx auth failure, known/low-priority, not tied to any specific incident**: all 3 crash-collector pods (one per node, sharing the single `rook-ceph-crash-collector-keyring` Secret) log a recurring `monclient(hunting): handle_auth_bad_method server allowed_methods [2] and I support [2,1]` followed by `[errno 13] RADOS permission denied (error connecting to the cluster)`. Confirmed 2026-09-16 this is **not** a stale/wrong key — `ceph auth get client.crash`'s key matches the Secret's key byte-for-byte — and **not** node-specific or tied to any particular incident (all 3 nodes show it, including nodes untouched by that day's work; deleting the pod to force a fresh session did not help, ruling out a simple stale-session cause too). Looks like an auth-method-negotiation issue between the `ceph-crash` daemon's client and this cluster's hardened mon config (`auth_allow_insecure_global_id_reclaim: false`), not a credentials problem — root cause not fully diagnosed. Impact is cosmetic: `ceph-crash` only uploads crash-dump reports to the mgr for observability, it doesn't touch cluster health/data/availability (`ceph -s` stays `HEALTH_OK` throughout). Deprioritized; revisit only if crash-dump visibility actually becomes needed, not proactively.
- **FreshRSS PVC ownership regression, 2026-09-16**: login started failing with "Error during context user init!" and the requirements self-check began failing every 30 minutes (`FreshRSS failed requirements: data/users/favicons/tokens need write permission`), right after the `docker.io/freshrss/freshrss` Renovate bump (1.29.1 → 1.30.0). Root cause: `data/`, `users/`, `favicons/`, `tokens/`, `extensions-data/`, `fever/`, `PubSubHubbub/`, `Retry-After/` were all `root:root` with no group/other access, while the app's actual Apache worker processes run as `www-data` (uid 33) — only `cache/` had the correct `root:www-data` group ownership (the working reference to diff against). Fixed live via `kubectl exec -c app -- chgrp -R www-data /var/www/FreshRSS/data /var/www/FreshRSS/data/*` — **this is a live PVC-data fix, not GitOps-tracked**; nothing in `kubernetes/apps/self-hosted/freshrss/` controls file ownership on already-provisioned data, so it will need re-applying if the pod ever lands on fresh/recreated storage. Watch for this same pattern (upstream image bump silently changing expected non-root ownership) on other apps after their next image bump — compare a known-working directory's ownership against the ones erroring, same method used here.
- **FreshRSS feed-fetch timeout, 2026-09-16**: one large podcast feed (`feeds.megaphone.fm/GLT1412515089`, 5.4MB) was intermittently failing with `invalid XML ... Invalid document end`, recurring every ~30min refresh cycle — a truncated-download symptom, not malformed XML at the source (re-fetching directly returned a complete, well-formed document). FreshRSS's own `'limits' => ['timeout' => 20]` in `data/config.php` was too short for this feed's size. Bumped to `60`. **Also a live, non-GitOps-tracked PVC edit** (`config.php` lives entirely on the PVC, not templated by the HelmRelease) — same re-apply caveat as above if the PVC is ever recreated. Verified via `php cli/actualize-user.php --user <name>` (manual feed refresh, avoids waiting for the next cron cycle).
...
**Recurrence confirmed 2026-09-16, not fully resolved**: the GSP fix reduces frequency but is not 100% reliable. First recurrence during the Talos 1.14.0->1.14.1 upgrade's node reboot (real incident, required physical power-cycle, recovered cleanly afterward). With Talos kernel-log forwarding now live (`talos/machineconfig.yaml.j2`'s `KmsgLogConfig`, see the observability/alloy app), deliberately ran two `just talos reboot-node 192.168.30.12` (`-m powercycle`) tests to gather real data instead of losing everything to the reboot:

- **Test 1**: graceful shutdown sequence captured only through phase 1 of 14 (`task stopAllPods: shutting down kubelet gracefully`) before the log connection died - ambiguous whether it hung during shutdown itself or a subsequent boot attempt. Stayed unreachable (`no route to host`) for 10+ minutes with no watchdog auto-recovery (same as the original incident - `iTCO_wdt`'s 5-minute timeout is not reliable here). Physical power-cycle recovered it cleanly; `ext-nvidia-cdi-gen` finished successfully on that boot.
- **Test 2**: graceful shutdown this time captured cleanly through **all 14 phases**, ending at `task reboot (1/1): starting` (the actual power-cycle action) - confirming the software shutdown sequence itself is not where the problem lives. The subsequent boot attempt sent **zero** kernel-log lines (no fresh NVIDIA driver-load messages, nothing) before the KVM console (directly observed this time) showed it locked at the Talos boot screen. Since kmsg forwarding requires networking to already be up, and networking comes up after kernel/driver init, this pins the hang to early boot/driver init - consistent with the original GSP RPC-deadlock theory, not a shutdown-path or late-boot issue. Physical power-cycle again recovered it cleanly.

Two-for-two reproductions via `-m powercycle` reboot, both requiring physical intervention (no auto-recovery in either case), both recovering cleanly afterward with a normal `ext-nvidia-cdi-gen` completion. This looks like a genuinely intermittent, low-frequency GSP-path failure that the 2026-07-30 fix mitigates but doesn't eliminate - **do not treat a k8s-2 reboot/upgrade as risk-free**; always run the standard `health-check` gate first and be prepared for a physical power-cycle. No further fix attempted yet (e.g. testing `NVreg_EnableGpuFirmware=0` isn't fully eliminating GSP behavior, or a driver version bump) - flagging as an open item, not actively being worked.
